...
function formatFallback(value, formatter) {
    if (typeof value === 'number')
        return formatter.numberToString(value, 2);
...
function formatValue(value, param, opt, formatter, formatEnum) {
    if (Array.isArray(value))
        return formatter.listToString(value.map((v) => formatValue(v, param, opt, formatter, formatEnum)));
    ...
    case 'lat':
    case 'lon':
        return formatFallback(value[opt], formatter);

    default: // unit
        return formatter.measureToString(value, 1, opt);
    }
}
...
function compile(string, options = {}) {
    ...
    return function(args) {
        ...
        for (let chunk of parsed) {
            ...
            buffer += formatValue(value, param, opt, formatter, formatEnum);
        }
